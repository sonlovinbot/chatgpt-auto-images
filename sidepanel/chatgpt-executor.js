...
export async function createChatGPTTab() {
  return chrome.tabs.create({ url: CHATGPT_URL, active: true });
}

export async function getCurrentOrExistingChatGPTTab() {
  const [active] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (active?.id && active.url?.startsWith(CHATGPT_URL)) return active;
...
export async function waitForTab(tabId, timeoutMs = 60000) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(new Error("ChatGPT did not finish loading within 60 seconds."));
    }, timeoutMs);
...
async function execute(tabId, func, args = []) {
  ...
  if (!results?.[0]) throw new Error("ChatGPT injection returned no result.");
  if (results[0].error) throw new Error(results[0].error.message || String(results[0].error));
  return results[0].result;
}

/**
 * Self-contained because Chrome serializes this function into ChatGPT's MAIN
...
async function prepareAndSubmitOnPage(task, config) {
  ...
  if (/verify you are human|checking your browser/i.test(pageText())) {
    return fail("VERIFICATION_REQUIRED", "session", "ChatGPT requires verification.");
  }
  if (!document.querySelector(selectors.composer)) {
    return fail("AUTH_REQUIRED", "session", "ChatGPT composer is unavailable. Sign in first.");
  }
    ...
    for (let index = 0; index < task.references.length; index += 1) {
      ...
      if (!input) {
        return fail(
          ...
          "upload",
          `ChatGPT file input was not found for reference ${index + 1}.`,
      ...
      if (!verified || verified.errorText) {
        return fail(
          ...
          "upload",
          verified?.errorText
            ? `ChatGPT rejected reference ${index + 1}: ${reference.name}.`
  ...
  if (!promptReady) {
    return fail("PROMPT_SYNC_FAILED", "composer", "ChatGPT editor did not retain the exact prompt.");
  }
  ...
  if (!accepted) {
    return fail(
      ...
      "submit",
      "Could not prove whether ChatGPT accepted the prompt.",
    );
  }
  record("submission-accepted", "ChatGPT accepted the prompt.", accepted);
  const recoverableConversationUrl = () => {
    try {
      const url = new URL(location.href);
      const match = url.pathname.match(/^\/c\/([^/]+)\/?$/i);
      return match?.[1] && !/^WEB:/i.test(match[1]) ? url.href : "";
    } catch {
      return "";
    }
  };
  const submittedFromNewChat = new URL(baseline.pageUrl).pathname === "/";
  if (submittedFromNewChat) {
    await waitFor(recoverableConversationUrl, 15000, 100);
  }
  baseline.pageUrl = location.href;
  baseline.recoveryUrl = recoverableConversationUrl();
  record("conversation-ready", "Recorded the post-submission conversation URL.", {
    pageUrl: baseline.pageUrl,
    recoveryUrl: baseline.recoveryUrl,
  });
...
async function collectResultOnPage(candidate) {
  ...
  if (!response.ok) return { ok: false, message: `Image request returned ${response.status}.` };
  const blob = await response.blob();
  if (!blob.type.startsWith("image/") || !blob.size) {
    return { ok: false, message: "Collected response is not a valid image." };
  }
...
export async function submitChatGPTTask(tabId, task, config) {
  return execute(tabId, prepareAndSubmitOnPage, [task, config]);
}

export async function scanChatGPTResult(tabId, baseline, config) {
  return execute(tabId, scanResultOnPage, [baseline, config]);
}

export async function collectChatGPTResult(tabId, candidate) {
