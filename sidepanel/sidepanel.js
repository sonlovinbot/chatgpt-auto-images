...
import {
  collectChatGPTResult,
  getCurrentOrExistingChatGPTTab,
  scanChatGPTResult,
  submitChatGPTTask,
  waitForTab,
} from "./chatgpt-executor.js";

const $ = (selector, root = document) => root.querySelector(selector);
const $$ = (selector, root = document) => [...root.querySelectorAll(selector)];
const QUEUE_KEY = "autoChatGPTImages.localQueue.v1";
const SETTINGS_KEY = "autoChatGPTImages.runnerSettings.v1";
const SESSIONS_KEY = "autoChatGPTImages.sessions.v1";
const DRAFT_KEY = "autoChatGPTImages.draft.v1";
const RUNNER_STATE_KEY = "autoChatGPTImages.runnerState.v1";
...
const DEFAULT_SETTINGS = {
  ...
  autoDownload: true,
  downloadMode: "auto",
  downloadFolder: "ChatGPT Image Automation",
};
...
function normalizeFilename(value, fallback = "chatgpt-image.png") {
...
async function updatePageGate() {
  ...
  const onChatGPT = Boolean(tab?.url?.startsWith("https://chatgpt.com/"));
  $("#mainHeader").hidden = !onChatGPT;
  $("#wrongPage").hidden = onChatGPT;
  $("#mainInterface").hidden = !onChatGPT;
  return onChatGPT;
}

async function moveToChatGPT() {
...
async function getSelectorConfig() {
  selectorConfigPromise ||= fetch(
    chrome.runtime.getURL("config/chatgpt-selectors.json"),
  ).then((response) => {
...
function renderQueue() {
  ...
  // Individual task errors remain visible on their queue cards and in logs.
  // The old global banner appeared for stale/manual failures and incorrectly
  // implied that VOX or the current ChatGPT page had just failed.
  $("#queueErrorBanner").hidden = true;
...
async function openConversationRecoveryTab(conversationUrl) {
  if (!isRecoverableChatGPTConversationUrl(conversationUrl)) {
    throw Object.assign(
      new Error(
        "Không thể mở lại conversation vì ChatGPT chưa cấp URL ổn định. " +
        "Extension sẽ không mở URL tạm hoặc gửi lại prompt.",
      ),
  ...
  if (!tab?.id) {
    throw Object.assign(new Error("Chrome không tạo được tab recovery ChatGPT."), {
  ...
  logSession("info", "session_start", {
    message: isRecovery
      ...
      : item.requiresFreshChat
        ? "Task retry sẽ mở New Chat mới; không dùng lại tab ChatGPT đã lỗi."
        : "Dùng tab ChatGPT hiện có, không reload trang.",
  });

  try {
    if (isVoxQueueItem(item) && item.pendingVoxResult?.blobId) {
      logSession("info", "vox_result_retry", {
        message: "Đã có image bytes; chỉ retry trả về VOX, không submit lại ChatGPT.",
      });
    ...
    if (isRecovery) {
      ...
      const recoveryUrl =
        item.resume.baseline.recoveryUrl ||
        (isRecoverableChatGPTConversationUrl(item.resume.baseline.pageUrl)
          ? item.resume.baseline.pageUrl
          : "");
      if (!tab?.id || !tab.url?.startsWith("https://chatgpt.com/")) {
        const matching = await chrome.tabs.query({ url: "https://chatgpt.com/*" });
        tab = recoveryUrl
          ? matching.find((candidate) => candidate.url === recoveryUrl) || null
          : null;
      }
      if (!tab?.id || !tab.url?.startsWith("https://chatgpt.com/")) {
        tab = await openConversationRecoveryTab(recoveryUrl);
        await updateQueueItem(item, {
          resume: {
            ...item.resume,
            tabId: tab.id,
      ...
      if (isRecoverableChatGPTConversationUrl(tab.url)) {
        item.resume.baseline.pageUrl = tab.url;
        item.resume.baseline.recoveryUrl = tab.url;
      }
      ...
      if (item.requiresFreshChat) {
        tab = await sendBackground("OPEN_FRESH_CHATGPT", {
        ...
        logSession("warn", "retry_new_chat_opened", {
          message:
            "Task chạy lại trong New Chat mới; không sử dụng tab ChatGPT đã lỗi.",
          details: {
            tabId: tab.id,
        ...
        tab = await getCurrentOrExistingChatGPTTab();
      }
      if (tab.status !== "complete") await waitForTab(tab.id);
      await pauseBoundary();
      ...
      const submittedPrompt = item.aspectRatio
        ...
        : item.prompt;
      submission = await submitChatGPTTask(
        tab.id,
        { prompt: submittedPrompt, references },
        config,
      );
      for (const entry of submission?.trace || []) {
        logSession(submission.ok ? "info" : "error", `[ChatGPT +${entry.elapsedMs}ms] ${entry.step}`, {
    ...
    logSession("info", "scanner_started", {
      message:
        "Theo dõi DOM mỗi giây; nhận ảnh mới đã tải đủ và ổn định, kể cả khi nút Stop của ChatGPT còn hiển thị.",
    });
    ...
    while (Date.now() < deadline && !candidate) {
      ...
      let scan;
      try {
        scan = await scanChatGPTResult(tab.id, submission.baseline, config);
      } catch (error) {
        ...
        tab = await openConversationRecoveryTab(
          submission.baseline.recoveryUrl || submission.baseline.pageUrl,
        );
        tabRecoveryCount += 1;
      ...
      if (scanCount === 1 || scanCount % 10 === 0) {
        ...
        if (
          liveTab?.url &&
          isRecoverableChatGPTConversationUrl(liveTab.url) &&
          submission.baseline.recoveryUrl !== liveTab.url
        ) {
          submission.baseline.pageUrl = liveTab.url;
          submission.baseline.recoveryUrl = liveTab.url;
          await updateQueueItem(item, {
            resume: {
              ...item.resume,
          ...
          logSession("info", "conversation_url_stabilized", {
            message: "Đã thay URL ChatGPT tạm bằng conversation URL có thể recovery.",
            details: { conversationUrl: liveTab.url },
          });
        }
      ...
      if (
        ...
        logSession("warn", "scanner_stalled", {
          message:
            "DOM ChatGPT không thay đổi trong 60 lần quét; đang mở lại conversation trong tab mới.",
          details: {
            previousTabId,
            conversationUrl: submission.baseline.pageUrl,
            submitCount: 0,
    ...
    if (!candidate) {
      throw Object.assign(
        new Error(`Không thấy ảnh ChatGPT mới sau ${runnerSettings.timeoutMinutes} phút.`),
    ...
    const result = await collectChatGPTResult(tab.id, candidate);
    if (!result?.ok) {
      throw Object.assign(
...
async function runQueue() {
  ...
  runner.running = true;
  runner.autoResumeRequested = true;
  runner.paused = false;
  ...
  try {
    ...
    runner.autoResumeRequested = false;
    runner.paused = false;
    runner.stopRequested = false;
    runner.currentItemId = "";
    await chrome.storage.local.set({
...
async function loadLocalState() {
  const stored = await chrome.storage.local.get({
    ...
    [SESSIONS_KEY]: [],
    [DRAFT_KEY]: {},
    [RUNNER_STATE_KEY]: { autoResume: false },
  });
  queue = Array.isArray(stored[QUEUE_KEY]) ? stored[QUEUE_KEY] : [];
  runner.autoResumeRequested = Boolean(stored[RUNNER_STATE_KEY]?.autoResume);
  ...
  $("#outputPrefix").value = draft.outputPrefix || "chatgpt-image";
  promptLines = [];
...
async function sendBackground(type, payload = {}) {
  const response = await chrome.runtime.sendMessage({
    scope: "auto-chatgpt-images:background",
    type,
    ...payload,
  });
  if (!response?.ok) {
...
function bindEvents() {
  ...
  $("#moveToChatGPT").addEventListener("click", () => void moveToChatGPT());
  ...
  $("#focusVoxAfterError").addEventListener("click", async () => {
    try {
      await sendBackground("FOCUS_VOX");
    } catch (error) {
      toast(error.message);
    }
  });
...
chrome.runtime.onMessage.addListener((message) => {
  if (
    message?.scope !== "auto-chatgpt-images:sidepanel" ||
