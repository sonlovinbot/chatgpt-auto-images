...
const executor = fs.readFileSync(
  new URL("../sidepanel/chatgpt-executor.js", import.meta.url),
  "utf8",
);
...
const selectors = JSON.parse(
  fs.readFileSync(
    new URL("../config/chatgpt-selectors.json", import.meta.url),
    "utf8",
  ),
);
...
test("decodes reference data URLs locally instead of fetching through ChatGPT CSP", () => {
...
test("panel exposes queue controls, session diagnostics and download setup", () => {
  ...
  assert.match(background, /case "OPEN_FRESH_CHATGPT":/);
  assert.match(background, /case "FOCUS_VOX":/);
});

test("reuses ChatGPT and gates the panel on other pages", () => {
  assert.match(executor, /getCurrentOrExistingChatGPTTab/);
  assert.match(panel, /id="wrongPage"/);
  assert.match(panel, /id="moveToChatGPT"/);
});
...
test("detects ChatGPT imagegen output from the live generated-image contract", () => {
  assert.equal(
...
test("accepts a fully loaded stable image even while ChatGPT Stop remains visible", () => {
...
test("uses stable ChatGPT file identity and only scans new assistant turns", () => {
  assert.match(executor, /chatgpt-file:\$\{fileId\}/);
...
test("recovers a submitted task by resuming the scanner without resubmitting", () => {
  ...
  assert.match(panelScript, /isRecoverableChatGPTConversationUrl/);
  assert.match(panelScript, /conversation_url_stabilized/);
  assert.match(executor, /!\/\^WEB:/);
});

test("continues a running queue automatically after the side panel reloads", () => {
  assert.match(panelScript, /RUNNER_STATE_KEY/);
  assert.match(panelScript, /autoResumeRequested/);
  assert.match(panelScript, /autoResume: true/);
  assert.match(panelScript, /runner\.autoResumeRequested &&/);
});

test("keeps stale failures on their task cards without a global error banner", () => {
  assert.match(panelScript, /\$\("#queueErrorBanner"\)\.hidden = true/);
});
