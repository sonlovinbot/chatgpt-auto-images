...
test("routes VOX START_BATCH to the proven side-panel executor only", () => {
  ...
  assert.match(startCase, /ensureChatGPTTab/);
...
test("every explicit retry opens a fresh ChatGPT tab before submission", () => {
  ...
  assert.match(execute, /sendBackground\("OPEN_FRESH_CHATGPT"/);
  assert.match(execute, /retry_new_chat_opened/);
  assert.ok(
    execute.indexOf('sendBackground("OPEN_FRESH_CHATGPT"') <
      execute.indexOf("submitChatGPTTask("),
  );
...
test("claims the exact queue task before touching ChatGPT", () => {
  const execute = between(
    panel,
    "async function executeQueueItem",
    "async function runQueue",
...
test("renews VOX waiting progress while the ChatGPT scanner is active", () => {
