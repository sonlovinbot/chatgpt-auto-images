(() => {
  const CONFIG_URL = chrome.runtime.getURL("config/chatgpt-selectors.json");
  let configPromise;
  ...
  async function detectBlockedState(config) {
    ...
    if (config.labels.verification.some((label) => text.includes(label))) {
      return { code: "VERIFICATION_REQUIRED", message: "ChatGPT requires browser verification." };
    }
    if (config.labels.usageLimit.some((label) => text.toLowerCase().includes(label.toLowerCase()))) {
      return { code: "USAGE_LIMIT_REACHED", message: "ChatGPT reports a usage limit." };
    }
    ...
    if (!composer && config.labels.login.some((label) => text.includes(label))) {
      return { code: "AUTH_REQUIRED", message: "Log in to ChatGPT to continue." };
    }
    return null;
  }
  ...
  async function uploadReferences(config, references) {
    ...
    if (!input) throw Object.assign(new Error("ChatGPT file input was not found."), { code: "CHATGPT_UI_UNSUPPORTED" });
  ...
  async function submitPrompt(config, prompt, promptHash) {
    ...
    const composer = document.querySelector(config.selectors.composer);
    if (!composer) throw Object.assign(new Error("ChatGPT composer was not found."), { code: "CHATGPT_UI_UNSUPPORTED" });
    ...
    if (!button || button.disabled) {
      throw Object.assign(new Error("ChatGPT send button is unavailable."), { code: "SUBMISSION_REJECTED" });
    }
    ...
    if (!userMessage) {
      throw Object.assign(new Error("Could not confirm whether ChatGPT accepted the prompt."), {
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.scope !== "auto-chatgpt-images:page") return;
