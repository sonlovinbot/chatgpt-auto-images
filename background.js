import { ALARM_NAME, CHATGPT_ORIGIN, ERROR_CODES, TASK_STATES } from "./src/constants.js";
...
const RESULT_DOWNLOAD_KEY = "autoChatGPTImages.armedResultDownload.v1";
...
async function pageMessage(tabId, type, payload = {}) {
  const response = await chrome.tabs.sendMessage(tabId, {
    scope: "auto-chatgpt-images:page",
    type,
    payload
  });
  if (!response?.ok) {
    const error = new Error(response?.error?.message || `ChatGPT page command failed: ${type}`);
...
async function ensureChatGPTTab(runtime, newConversation = false) {
  if (!newConversation && runtime.chatGPTTabId) {
    const existing = await chrome.tabs.get(runtime.chatGPTTabId).catch(() => null);
    if (existing?.url?.startsWith(CHATGPT_ORIGIN)) {
      if (runtime.focusChatGPTRequested) {
        await chrome.tabs.update(existing.id, { active: true });
        await chrome.windows.update(existing.windowId, { focused: true }).catch(() => {});
        await saveRuntime({ focusChatGPTRequested: false });
      }
      return existing;
    }
  }
  ...
  if (!newConversation && tabs[0]) {
    ...
    await saveRuntime({ chatGPTTabId: tabs[0].id, conversationUrl: tabs[0].url });
    return tabs[0];
  }
  const tab = await chrome.tabs.create({ url: CHATGPT_ORIGIN, active: true });
  await saveRuntime({ chatGPTTabId: tab.id, conversationUrl: tab.url });
  return tab;
}
...
async function waitForPage(tabId, timeoutMs = 30000) {
  ...
  const error = new Error("ChatGPT tab did not become ready.");
  error.code = ERROR_CODES.CHATGPT_UI_UNSUPPORTED;
  throw error;
}
...
async function runManualTask(payload) {
  ...
  try {
    ...
    const runtime = await getRuntime();
    const tab = await ensureChatGPTTab({ ...runtime, focusChatGPTRequested: true }, true);
...
function scheduleManualTask(payload) {
  if (manualRunnerPromise) {
    throw new Error("A manual ChatGPT task is already running.");
  }
...
async function runBatch() {
  ...
  try {
    ...
    const tab = await ensureChatGPTTab(runtime, !runtime.conversationUrl);
    const page = await waitForPage(tab.id);
    if (page.blocked) throw Object.assign(new Error(page.blocked.message), { code: page.blocked.code });
...
async function handleMessage(message, sender) {
  switch (message?.type) {
    ...
    case "OPEN_DOWNLOAD_SETTINGS":
      ...
      return { ok: true };
    case "OPEN_FRESH_CHATGPT": {
      const runtime = await getRuntime();
      const tab = await ensureChatGPTTab(
        { ...runtime, focusChatGPTRequested: true },
        true,
      );
    ...
    case "START_BATCH": {
      ...
      await saveRuntime({
        ...
        focusChatGPTRequested: Boolean(message.initiatedByVox)
      });
      ...
      const panelOpened = await panelOpenPromise;
      {
        ...
        await ensureChatGPTTab(
          { ...current, focusChatGPTRequested: true },
          Boolean(current.voxOpenNewChat) && !sameBatch,
        );
      }
      await chrome.runtime.sendMessage({
        scope: "auto-chatgpt-images:sidepanel",
    ...
    case "FOCUS_CHATGPT": {
      const runtime = await getRuntime();
      if (runtime.chatGPTTabId) await chrome.tabs.update(runtime.chatGPTTabId, { active: true });
      return { ok: true };
    }
    default:
      return null;
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.scope !== "auto-chatgpt-images:background") return;
